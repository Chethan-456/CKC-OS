...
// ── AI Chat via Groq, proxied through our own Vercel serverless function ──
// The Groq key lives server-side only (GROQ_API_KEY in Vercel env vars),
// so it is never sent to, or visible in, the browser.
async function askGroq(messages, lang, userCode) {
  const response = await fetch("/api/groq-debug", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      messages,
      langName: LANGS[lang]?.n || lang,
      userCode,
    }),
  });
  ...
  return data.content || "No response from Groq.";
}
