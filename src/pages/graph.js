...
const GROQ_MODEL   = "llama-3.3-70b-versatile"; // alternatives: "mixtral-8x7b-32768", "gemma2-9b-it"
...
${trimmed}`;
    ...
    try {
      ...
      const res = await fetch(GROQ_API_URL, {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: "system",
              content: "You are a code analyzer. Respond ONLY with raw JSON. No markdown, no backticks, no explanation."
            },
            {
              role: "user",
              content: prompt
      ...
      if (!graph.concepts && !graph.errors) throw new Error("Invalid graph format returned");
