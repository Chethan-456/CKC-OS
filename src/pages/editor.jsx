...
function Shell({ user, onLogout }) {
  ...
  // ── Yjs: one Y.Doc + Supabase-backed provider per session ──
  const ydocRef = useRef(null);
  const providerRef = useRef(null);
  if (!ydocRef.current) ydocRef.current = new Y.Doc();

  const getYText = useCallback((fileKey) => ydocRef.current.getText(fileKey), []);
  ...
  useEffect(() => {
    ...
    const updatePresence = () => {
      ...
      setConnectedCount(states.length || 1);
      setCursors(
        states
  ...
  useEffect(() => {
    const onUpdate = (_update, origin) => {
      if (origin) return; // local op, already reflected by typing
      setOpCnt(c => c + 1);
    };
  ...
  const handleCursorMove = useCallback((line, col) => {
    setCursor({ line, col });
    providerRef.current?.awareness.setLocalStateField("cursor", { line, col, lang, tabId: activeTab });
  }, [lang, activeTab]);
    ...
    <>
      ...
      <div className="topbar">
        ...
        <div className="av-group" style={{ display: "flex", alignItems: "center", gap: -5, marginLeft: 8 }}>
          {[{ id: me.id, name: me.name, color: me.color, bg: me.bg }, ...cursors].map((b, i) => (
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "8px" }}>
            {rpTab === "crdt" ? (
              <div style={{ padding: "20px", textAlign: "center", color: "#4a5568", fontSize: 11 }}>
                {opCnt > 0 ? `${opCnt} Yjs update(s) applied this session` : "Waiting for ops.."}
              </div>
            ) : (
